...
import {
  GITHUB_API_RESPONSE_CONTENT_TYPE,
  GITHUB_API_VERSION,
  GITHUB_API_VERSION_HEADER_NAME,
  GITHUB_API_STANDARD_HEADERS,
} from "./constants.js";
import { PackageType } from "./github-cli-types.js";
import { Octokit } from "@octokit/core";
import { SemVer } from "semver";

export async function deletePackageVersionAsync(
  orgId: string,
  packageId: string,
  version: SemVer,
  ...
  return new Promise<void>(async (resolve, reject) => {
    ...
    try {
      ...
      var versions = await octokit.request(
      ...
      var versionToDelete = versions.data.find(
        (v) => v.name == version.format(),
      );

      const versionId = versionToDelete?.id;

      if (versionId && versionId != undefined) {
        var deleteResponse = await octokit.request(
        ...
        if (deleteResponse.status != 204) {
          console.log(
            `The package version was not deleted because the server encountered an error: ${deleteResponse.status}`,
          );
          reject(deleteResponse.status);
        }
      } else {
        console.log(
          `The package version ${version} was not found. Skipping...`,
        );
      }
      resolve();
    } catch (ex) {
...
export async function deletePackageAsync(
  orgId: string,
  packageId: string,
  token: string | undefined,
): Promise<void> {
  return new Promise<void>(async (resolve, reject) => {
    ...
    try {
      ...
      if (deleteResponse.status == 204) {
        ...
        console.log(
          `The package ${packageId} was not deleted because the server encountered an error: ${deleteResponse.status}`,
        );
        reject(deleteResponse.status);
      }
    } catch (ex) {
